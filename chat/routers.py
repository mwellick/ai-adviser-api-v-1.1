...
@chats_router.post(
    "/guest/create/",
    response_model=GuestChatCreated,
    status_code=status.HTTP_201_CREATED,
)
async def create_guest_chat(
    db: db_dependency, response: Response, chat: GuestChatCreate
):
    chat_instance = await guest_chat_create(db, chat)

    if chat.user_id is None:
        chat_id = random.randint(1, 100)
        chat_data = {"chat_id": chat_id, "theme_id": chat_instance.theme_id}
...
@chats_router.post(
    "/create/", response_model=ChatCreated, status_code=status.HTTP_201_CREATED
)
...
@chats_router.get(
    "/{chat_id}/", response_model=RetrieveChat, status_code=status.HTTP_200_OK
)
async def retrieve_chat(
    user: user_dependency, db: db_dependency, chat_id: int = Path(gt=0)
):
