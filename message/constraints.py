...
async def check_db_saved_messages(user: user_dependency, db: db_dependency):
    ...
    if not saved:
        raise HTTPException(
            detail="Not found any saved messages", status_code=status.HTTP_404_NOT_FOUND
        )
...
async def check_existing_message(
    user: user_dependency, db: db_dependency, message_id: int
):
    query = (
        select(Message)
        .options(joinedload(Message.chat).joinedload(Chat.user))
        .join(Chat)
        .where(Chat.user_id == user.get("id"))
...
async def check_existing_saved_message(
    ...
    if not saved_message:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Saved message not found"
        )
